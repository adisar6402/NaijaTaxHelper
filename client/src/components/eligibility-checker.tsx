...
export default function EligibilityChecker() {
  ...
  const checkEligibility = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("POST", "/api/check-eligibility", formData);
    ...
    <section id="checker" className="py-16 bg-[var(--nigeria-gray)]">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12 fade-in">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Tax Eligibility Checker</h2>
          <p className="text-xl text-gray-600">Find out which tax exemptions you qualify for in under 2 minutes</p>
        </div>
          ...
          <CardContent className="p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  />
                  <p className="text-sm text-gray-500 mt-1">Enter your total yearly income before taxes</p>
                </div>
                ...
                <div>
                  ...
                  />
                  <p className="text-sm text-gray-500 mt-1">Total rent you pay per year (for stamp duty exemption)</p>
                </div>
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setFormData({ ...formData, isRemoteWorker: checked as boolean })}
                />
            ...
            {results && (
              <div className="mt-8 p-6 bg-gray-50 rounded-lg">
                <h3 className="text-xl font-semibold mb-4">Your Tax Exemption Status</h3>
                
                <div className="space-y-4">
                  <div className={`flex items-center p-4 border rounded-lg ${
                    results.payeExempt ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'
                  }`}>
                    ...
                    <div>
                      <h4 className={`font-semibold ${results.payeExempt ? 'text-green-800' : 'text-red-800'}`}>
                        PAYE Tax Exemption
                      </h4>
                      <p className={results.payeExempt ? 'text-green-700' : 'text-red-700'}>
                        {results.payeExempt 
                      ...
                      {results.payeExempt && results.estimatedSavings > 0 && (
                        <p className="text-sm text-green-600 mt-1">
                          Estimated savings: ₦{results.estimatedSavings.toLocaleString()}/year
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className={`flex items-center p-4 border rounded-lg ${
                    results.stampDutyExempt ? 'bg-blue-50 border-blue-200' : 'bg-red-50 border-red-200'
                  }`}>
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-4 ${
                    ...
                    <div>
                      <h4 className={`font-semibold ${results.stampDutyExempt ? 'text-blue-800' : 'text-red-800'}`}>
                        Stamp Duty Exemption
                      </h4>
                      <p className={results.stampDutyExempt ? 'text-blue-700' : 'text-red-700'}>
